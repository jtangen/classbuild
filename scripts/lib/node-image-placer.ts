/**
 * Node-compatible version of src/services/gemini/imagePlacer.ts.
 * Replaces <figure class="gemini-image"> placeholders with actual Gemini-generated images.
 * Uses generateInfographicNode (raw base64) instead of browser canvas compression.
 */
import { generateInfographicNode } from './node-image';

async function generateWithRetry(prompt: string, apiKey: string, maxRetries = 3): Promise<string> {
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      const { base64, mimeType } = await generateInfographicNode(prompt, apiKey);
...
export async function replaceGeminiImagePlaceholdersNode(html: string, apiKey: string): Promise<string> {
