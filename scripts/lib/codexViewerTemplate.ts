/**
 * Codex course viewer — the published per-course index.html.
 *
 * Chrome (sidebar, tab bar, cards, welcome) is the ClassBuild "Codex" Parchment
 * design system: cream ground, ink prose in Newsreader, maroon + gilt accents,
 * JetBrains Mono metadata. Reading / Practice Quiz / Weekly Challenge load the
 * already-themed course HTML in sandboxed iframes (so each course keeps its own
 * Terminal/Studio/Almanac/… look). Slides, Audio, Discussion and Activities are
 * rendered natively in Codex.
 *
 * Showcase tab order: Reading · Slides · Audio · Practice Quiz · Discussion ·
 * Activities · Weekly Challenge. A tab appears only when its data exists.
 */
...
<meta name="description" content="${esc(clamp(course.courseOverview || course.courseTitle, 155))}">
...
<style>
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
  ...
  .back-link{display:inline-flex;align-items:center;gap:.45rem;font-family:var(--mono);font-size:10.5px;font-weight:500;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);text-decoration:none;margin-bottom:1.3rem;transition:color .16s}
  .back-link:hover{color:var(--maroon)}
  .back-arrow{font-size:1.05em;transition:transform .16s}
...
<nav class="sidebar">
  <div class="side-head">
    ...
    <a class="back-link" href="/"><span class="back-arrow" aria-hidden="true">←</span>All example courses</a>
...
<script>
  ...
  function showChapter(n){
    ...
    if(panel){var tabs=panel.querySelectorAll('.tab');var panes=panel.querySelectorAll('.tab-pane');
      ...
      // resize any iframe in the first (visible) pane
